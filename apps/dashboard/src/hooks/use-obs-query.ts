...
export function useOBSScenes() {
  ...
  const setCurrentScene = useMutation<
    { success: boolean },
    Error,
    string,
    { previous: OBSState['scenes'] | undefined }
  >({
    mutationFn: (sceneName) => trpcClient.control.obs.scenes.setCurrentScene.mutate({ sceneName }),
  ...
  const setPreviewScene = useMutation<
    { success: boolean },
    Error,
    string,
    { previous: OBSState['scenes'] | undefined }
  >({
    mutationFn: (sceneName) => trpcClient.control.obs.scenes.setPreviewScene.mutate({ sceneName }),
    onMutate: async (sceneName) => {
      await queryClient.cancelQueries({ queryKey: obsKeys.scenes() })
      const previous = queryClient.getQueryData<OBSState['scenes']>(obsKeys.scenes())
...
export function useOBSStudioMode() {
  ...
  const setEnabled = useMutation<
    ...
    Error,
    boolean,
    { previous: OBSState['studioMode'] | undefined }
  >({
    mutationFn: (enabled) => trpcClient.control.obs.studioMode.setEnabled.mutate({ enabled }),
