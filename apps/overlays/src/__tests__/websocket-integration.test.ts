...
const createMockSocket = (options: {
  ...
  return { mockSocket, mockChannel, getConnectionAttempts: () => connectionAttempts, getMessagesSent: () => messagesSent }
}
...
describe('WebSocket Integration & End-to-End Resilience', () => {
  ...
  describe('Production Streaming Scenarios', () => {
    test('maintains overlay functionality during 30-second network interruption', async () => {
      const { mockSocket, mockChannel } = createMockSocket({
      ...
      ;(mockSocket.isConnected as Mock).mockImplementation(() => {
        ...
        if (reconnectionPhase < 300) { // 30 seconds of interruption (100ms intervals)
          reconnectionPhase++
    ...
    test('handles high-frequency state updates during live streaming', async () => {
      ...
      const stateHandler = (mockChannel.on as Mock).mock.calls.find(
        call => call[0] === 'stream_state'
      )?.[1]
    ...
    test('recovers gracefully from Phoenix server restart', async () => {
      ...
      const { mockSocket: initialSocket, mockChannel: initialChannel } = createMockSocket({})
      const { mockSocket: restartedSocket, mockChannel: restartedChannel } = createMockSocket({
  ...
  describe('Message Queue Resilience', () => {
    ...
    test('preserves message order during reconnection', async () => {
      const { mockSocket, mockChannel, getMessagesSent } = createMockSocket({})
  ...
  describe('Error Recovery Patterns', () => {
    test('recovers from malformed message handling', async () => {
      ...
      const contentHandler = (mockChannel.on as Mock).mock.calls.find(
        call => call[0] === 'content_update'
      )?.[1]
