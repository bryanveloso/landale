...
class TestAudioBufferMemoryBounds:
    ...
    async def test_buffer_size_limit_enforcement(self, audio_processor, audio_format):
        ...
        initial_buffer_size = audio_processor.buffer.total_size
    ...
    async def test_memory_leak_during_continuous_operation(self, audio_processor, audio_format, audio_generator):
        ...
        assert memory_growth < max_allowed_growth, (
            ...
            f"during {operation_cycles} cycles"
        )
    ...
    async def test_memory_pressure_under_high_input_rate(self, audio_processor, audio_format):
        ...
        memory_stats = audio_processor.get_memory_stats()
    ...
    async def test_buffer_cleanup_after_processing(self, audio_processor, audio_format):
        ...
        with patch.object(audio_processor, '_combine_chunks', return_value=b'test_data'):
            with patch.object(audio_processor, '_pcm_to_float32', return_value=np.zeros(16000)):
                with patch.object(audio_processor, '_parse_whisper_output', return_value=mock_event):
                    with patch('subprocess.run') as mock_subprocess:
                        ...
                        # Force processing
                        initial_buffer_size = len(audio_processor.buffer.chunks)
...
class TestAudioBufferResourceCleanup:
    ...
    async def test_temporary_file_cleanup_on_error(self, processor_with_mocks):
        ...
        with patch('tempfile.NamedTemporaryFile', side_effect=mock_named_temp_file):
            with patch('os.unlink', side_effect=mock_unlink):
                with patch('subprocess.run', side_effect=Exception("Whisper failed")):
    ...
    async def test_memory_optimization_resource_cleanup(self, processor_with_mocks):
        ...
        with patch('subprocess.run') as mock_subprocess:
            ...
            initial_memory = processor_with_mocks.get_memory_usage()
            ...
            # Memory should have been processed and cleaned up
            final_memory = processor_with_mocks.get_memory_usage()
