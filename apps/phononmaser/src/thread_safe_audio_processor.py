"""Thread-safe audio processor with real asyncio locks for concurrent operations."""
...
class ThreadSafeAudioProcessor:
    """Thread-safe audio processor with asyncio locks."""
    ...
    async def start(self) -> None:
        ...
        async with self._buffer_lock:
            ...
            logger.info("Thread-safe audio processor started")
    ...
    async def stop(self) -> None:
        ...
        logger.info("Thread-safe audio processor stopped")

    async def add_chunk(self, chunk: AudioChunk) -> None:
        """Add an audio chunk to the buffer in thread-safe manner."""
        async with self._buffer_lock:
            ...
            duration = await self._get_buffer_duration_unsafe()
    ...
    async def get_buffer_duration(self) -> float:
        """Get buffer duration in seconds (thread-safe)."""
        async with self._buffer_lock:
            return await self._get_buffer_duration_unsafe()

    async def _get_buffer_duration_unsafe(self) -> float:
        """Get buffer duration without acquiring lock (internal use only)."""
        if not self.buffer.chunks:
            return 0.0
    ...
    async def _processing_loop(self) -> None:
        ...
        while self.is_running:
            if await self._should_process_buffer():
                logger.info(f"Processing buffer with {await self.get_buffer_duration():.1f}s of audio")
    ...
    async def _should_process_buffer(self) -> bool:
        """Check if buffer should be processed (thread-safe)."""
        async with self._buffer_lock:
            if not self.buffer.chunks:
                return False
    ...
    async def _process_buffer(self) -> TranscriptionEvent | None:
        """Process the current buffer (thread-safe)."""
        async with self._processing_lock:
