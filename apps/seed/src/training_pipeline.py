...
class TrainingDataPipeline:
    ...
    async def prepare_conversation_dataset(
        ...
        filtered_contexts = [
            ...
            if len(ctx.get('transcript', '').split()) >= min_context_length
        ]
    ...
    async def prepare_pattern_dataset(
        ...
        pattern_contexts = [
            ctx for ctx in contexts 
            if ctx.get('patterns') or ctx.get('sentiment')
        ]
    ...
    def _format_conversation_entry(self, context: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def _format_pattern_entry(self, context: Dict[str, Any], include_dynamics: bool = True) -> Optional[Dict[str, Any]]:
    ...
    def _format_temporal_entry(self, sequence: List[Dict[str, Any]], session: str) -> Optional[Dict[str, Any]]:
