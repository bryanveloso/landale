...
class DatasetExporter:
    ...
    async def export_huggingface_dataset(
        ...
        days_back: int = 30,
        split_ratio: Dict[str, float] = {"train": 0.8, "validation": 0.2}
    ) -> Dict[str, str]:
        """
        ...
        config = {
            "dataset_info": {
                ...
                "download_size": sum(
                    Path(f).stat().st_size for f in [train_file, val_file]
                ),
                "dataset_size": sum(
                    Path(f).stat().st_size for f in [train_file, val_file]
                )
            ...
            for i, line in enumerate(f):
                ...
                openai_entry = {
                    "messages": [
                        ...
                        {
                            "role": "user", 
                            "content": f"Analyze this stream moment: {data['input']}"
                        },
                        {
                            "role": "assistant",
                            "content": self._generate_assistant_response(data)
                        }
                    ]
                }
    ...
    async def export_csv_format(
        ...
        with open(base_file, 'r', encoding='utf-8') as jsonl_file:
    ...
    async def export_training_summary(
        self,
        days_back: int = 30
    ) -> str:
        """
