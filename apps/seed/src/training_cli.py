...
Examples:
  ...
  python -m seed.training_cli export --format openai --session stream_2024_01_15
    ...
    prepare_parser.add_argument(
        "--days", type=int, default=30, help="Number of days back to collect data (default: 30)"
    )
    prepare_parser.add_argument("--session", help="Filter by specific session ID (e.g., stream_2024_01_15)")
    prepare_parser.add_argument(
        "--min-length", type=int, default=10, help="Minimum word count for transcript inclusion (default: 10)"
    )

    # Export command
    export_parser = subparsers.add_parser("export", help="Export datasets in specific formats")
    export_parser.add_argument(
        "--format", choices=["huggingface", "openai", "csv"], required=True, help="Export format"
    )
    export_parser.add_argument(
        "--type",
        choices=["conversation", "pattern", "multimodal", "temporal"],
    ...
    export_parser.add_argument("--days", type=int, default=30, help="Number of days back to collect data (default: 30)")
    export_parser.add_argument("--session", help="Filter by specific session ID")
    export_parser.add_argument(
        "--max-examples", type=int, default=1000, help="Maximum number of examples for OpenAI format (default: 1000)"
    )
    export_parser.add_argument(
        "--train-split", type=float, default=0.8, help="Training split ratio for Hugging Face format (default: 0.8)"
    )
    ...
    summary_parser.add_argument("--days", type=int, default=30, help="Number of days back to analyze (default: 30)")
...
async def handle_prepare_command(args, pipeline: TrainingDataPipeline):
    ...
    if args.type == "conversation":
        filepath = await pipeline.prepare_conversation_dataset(
            session_filter=args.session, days_back=args.days, min_context_length=args.min_length
        )
    elif args.type == "pattern":
        filepath = await pipeline.prepare_pattern_dataset(session_filter=args.session, days_back=args.days)
    elif args.type == "multimodal":
        filepath = await pipeline.prepare_multimodal_dataset(session_filter=args.session, days_back=args.days)
    elif args.type == "temporal":
        filepath = await pipeline.prepare_temporal_dataset(session_filter=args.session, days_back=args.days)
    else:
...
async def handle_export_command(args, exporter: DatasetExporter):
    ...
    if args.format == "huggingface":
        ...
        files = await exporter.export_huggingface_dataset(
            dataset_type=args.type, session_filter=args.session, days_back=args.days, split_ratio=split_ratio
        )
        ...
        filepath = await exporter.export_openai_format(
            session_filter=args.session, days_back=args.days, max_examples=args.max_examples
        )
        ...
        filepath = await exporter.export_csv_format(
            dataset_type=args.type, session_filter=args.session, days_back=args.days
        )
...
async def handle_stats_command(_args, pipeline: TrainingDataPipeline):
