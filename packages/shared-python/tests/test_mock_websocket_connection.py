...
class MockWebSocketConnection:
    ...
    def __init__(
        ...
        fail_after_n_messages: int | None = None,
    ...
    async def ping(self) -> None:
        ...
        if self.fail_heartbeat_after and self.ping_count >= self.fail_heartbeat_after:
            raise ConnectionClosed(None, None)
    ...
    def add_phoenix_message(self, topic: str, event: str, payload: dict[str, Any], ref: str = "1"):
        ...
        """
        message = json.dumps({"topic": topic, "event": event, "payload": payload, "ref": ref})
    ...
    def get_sent_message(self, index: int) -> dict[str, Any] | None:
        """
...
class MockWebSocketFactory:
    ...
    def create_unstable_connection() -> MockWebSocketConnection:
        ...
        return MockWebSocketConnection(simulate_network_issues=True, fail_after_n_messages=5)

    @staticmethod
    def create_slow_connection() -> MockWebSocketConnection:
        """Create a slow connection with high latency."""
        return MockWebSocketConnection(connection_delay=0.2, message_delay=0.05)

    @staticmethod
    def create_heartbeat_failing_connection() -> MockWebSocketConnection:
        """Create a connection that fails heartbeat checks."""
        return MockWebSocketConnection(fail_ping=True, fail_heartbeat_after=3)
...
def create_phoenix_chat_message(username: str, message: str, **kwargs) -> str:
    ...
    return json.dumps({"topic": "events:all", "event": "chat_message", "payload": {"data": data}})
...
class TestMockWebSocketConnection:
    ...
    async def test_phoenix_message_helpers(self, mock_websocket):
        ...
        mock_websocket.add_phoenix_message("test:channel", "test_event", {"data": "test"}, "ref1")
