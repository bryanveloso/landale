                ...
                while attempts <= retry_attempts:
                    try:
                        ...
                        if attempts < retry_attempts:
                            ...
                            logger.info(
                                f"Retrying {func.__name__} in {delay:.1f}s (attempt {attempts}/{retry_attempts})"
                            )
...
def safe_handler(func: Callable) -> Callable:
    ...
    """
    return error_boundary(log_level=logging.WARNING, reraise=False, default_return=None)(func)


def retriable_network_call(max_attempts: int = 3, base_delay: float = 1.0) -> Callable:
...
def critical_operation(func: Callable) -> Callable:
    ...
    """
    return error_boundary(log_level=logging.ERROR, reraise=True)(func)
