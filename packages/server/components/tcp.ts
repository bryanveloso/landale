...
const handleData = async (socket: Socket<{}>, data: Buffer, wss: Server) => {
  ...
  while (buffer.length > 0) {
    ...
    if (isNaN(length)) {
      console.error(`🔀 TCP: Invalid message length: ${buffer.slice(0, space)}`)
      buffer = ''
      break
    }
    ...
    if (buffer.length >= endIndex) {
      ...
      console.log(`🔀 TCP: Received data from ${socket.remoteAddress}: ${message}`)
...
const handleError = (socket: Socket<{}>, error: Error) => {
  console.error(`🔀 TCP: Socket error from ${socket.remoteAddress}: ${error.message}`)
}
