...
const handleMessage = (ws: ServerWebSocket<{}>, server: Server, message: string | Buffer) => {
  if (message === 'ping') {
    ws.send('pong')
  } else {
...
const handleClose = (ws: ServerWebSocket<{}>, code: number, message: string) => {
  console.log(`📶 WSS: Socket closed by ${ws.remoteAddress} with code ${code}: ${message}`)
}
