...
export function StreamStatus() {
  ...
  const { isConnected: onlineConnected, error: onlineError } = useSubscription(
    'twitch.onStreamOnline',
    undefined,
    {
  ...
  const { isConnected: offlineConnected, error: offlineError } = useSubscription(
    'twitch.onStreamOffline',
    undefined,
    {
        ...
        <div className="flex items-center gap-3">
          <div 
            className={`h-4 w-4 rounded-full ${
              streamStatus.isLive ? 'bg-red-500 animate-pulse' : 'bg-gray-500'
            }`} 
          />
          <span className={`text-lg font-medium ${
            streamStatus.isLive ? 'text-red-400' : 'text-gray-400'
          }`}>
          ...
          <div className="mt-4 space-y-2">
            ...
            {streamStatus.uptime && (
              <div>
                ...
                <span className="text-sm text-gray-200 font-mono">{streamStatus.uptime}</span>
              </div>
            )}
            ...
            {streamStatus.startedAt && (
              <div>
                ...
                <span className="text-sm text-gray-200">
                  {streamStatus.startedAt.toLocaleTimeString()}
                </span>
              </div>
            )}
          </div>
